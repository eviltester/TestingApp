...
function PulperApi(XApiAuthCookieValue){
    ...
    this.createBook = function (title, authorid, publisherid, seriesid, serieslabel, year,
        ...
        var authors = [];
        if(existsWithValue(authorid)){
            if(Array.isArray(authorid)){
                for(authorindex = 0; authorindex < authorid.length; authorindex++){
                    authors.push({"id": authorid[authorindex]});
                }
            }else {
                authors.push({"id": authorid});
            }
        }
        obj.authors = authors;
        // todo support house author
    ...
    this.amendBook = function (bookid, title, authorid, houseauthorid, publisherid, seriesid, serieslabel, year,
                                renderResults, renderErrorMessage){
        var obj = {}; // {"id": bookid}; do not add id in body for PUT
        ...
        obj.authors = authors;
        if(existsWithValue(houseauthorid)){obj.houseAuthor = {"id" : houseauthorid}; }
        ...
        this.put(aUrl, obj, renderResults, renderErrorMessage);
    }
    ...
    this.put = function (aUrl, obj, renderResults, renderErrorMessage){

        this.xhttpapi.putRequest(aUrl, obj)
            .then(
                function(response) {
                    if(response.data) {
                        renderResults(response.data);
